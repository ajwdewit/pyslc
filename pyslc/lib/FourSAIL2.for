...
c	May 2006
c
c	Modified crown weight to approach Fcd again, but to make it go to one for large zenith angles
c	The new factor is called Fcdc
c	May 2008
c
c
c     Input parameters :
...
c
c       Modified by Allard de Wit 2008-08-13
c       force delta_geom to be true so angular parameters are recalculated
c       for each run. This makes the code more robust, although it loses a 
c       factor three in speed.
c
c       Original code:
c	delta_geom=(tts.ne.tts_old).or.(tto.ne.tto_old).
c     +				or.(psi.ne.psi_old)
c
c       New code:
	delta_geom=(.true.)
...
c
		...
		Fcdc=1-(1-Fcd)**(.5/cts+.5/cto)			!New crown weight factor
c
	end if
c	WRITE(*,*) tants, tanto, cspsi, dso, Cs, Co, Overlap
c
...
c
c       Original code:
c       s=(1.-diss)*fbu*(1.-fbu)
c
c       rho1=((1-fbu-s)*rg+s*rb)/(1-fbu)
c       tau1=((1-fbu-s)*tg+s*tb)/(1-fbu)
c       rho2=(s*rg+(fbu-s)*rb)/fbu
c       tau2=(s*tg+(fbu-s)*tb)/fbu
c
c       New code: Updated by Valerie on 04/07/2011, following Wout's email of 01/07/2011
	f1b=(1-diss)*fbu
	f1g=1-f1b
	f2g=(1-diss)*(1-fbu)
	f2b=1-f2g
...
c	New crown factor applied to rso (W. Verhoef, May 2008)
